...
def index():
    ...
    # Exécution normale
    if lang == 'bzh' or lang == 'br':
        return render_template('./indexen.html')
    elif lang == 'fr' or lang == None:
        return render_template('./index.html')
    # Sortie du cadre
    else:
        # Est-ce que le paramètre est ISO-639-1 ?
        for p_id in iso:
            c_id = p_id.get('code')
            n_id = p_id.get('name')
            if c_id == lang:
                return render_template('./404.html', n_id=n_id)
        # Si ce n'est pas un code de langue c'est autre chose
        # Je teste pour savoir si c'est du code exécutable
        try:
            stack = exe(lang)
            if stack == 0:
                return render_template('./4031.html')
            else:
                return render_template('./4032.html', stack=error)
        except:
            # Si ça n'en est pas, je me fiche de l'utilisateur
